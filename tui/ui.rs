...
fn draw_overview(frame: &mut Frame, app: &mut App, area: Rect) {
    let status_str = format!("Status: Healthy | ARP Attacks: 0 | Suspicious Packets: 3");
    let text = vec![
        text::Line::from(""),
...
fn draw_logger(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let logs: Vec<ListItem> = app
        ...
        .collect();
    let logs = List::new(logs).block(Block::bordered().title(Span::styled(
                "Logs",
                Style::default()
                .fg(Color::Magenta)
                .add_modifier(Modifier::BOLD),
...
fn draw_control_panel(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let tasks: Vec<ListItem> = app
        ...
        .collect();
    let tasks = List::new(tasks).block(Block::bordered().title(Span::styled(
                "Controls",
                Style::default()
                .fg(Color::Magenta)
                .add_modifier(Modifier::BOLD),
