...
pub fn build_ebpf(packages: impl IntoIterator<Item = Package>) -> Result<()> {
    ...
    } in packages
    {
        ...
        let rustflags = "-C debuginfo=2 -C link-arg=--btf";
...
fn main() -> anyhow::Result<()> {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|cargo_metadata::Package { name, .. }| name == "anti-arp-spoof-ebpf")
        .ok_or_else(|| anyhow!("anti-arp-spoof-ebpf package not found"))?;
